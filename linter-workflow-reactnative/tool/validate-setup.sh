...
echo "🔍 Validating Outcode React Native Code Quality setup..."
echo ""
...
# Check for React Native CLI or Expo CLI (either is fine)
if command -v react-native &> /dev/null || command -v expo &> /dev/null || grep -q "react-native" package.json 2>/dev/null; then
  if command -v react-native &> /dev/null; then
    check_command "react-native" "React Native CLI"
  elif command -v expo &> /dev/null; then
    check_command "expo" "Expo CLI"
  else
    echo -e "${YELLOW}⚠️${NC}  React Native CLI: Not found (but react-native in dependencies)"
    echo -e "   ${BLUE}ℹ️${NC}  This is fine if using Expo or Metro bundler directly"
    ((WARNINGS++))
  fi
else
  echo -e "${YELLOW}⚠️${NC}  React Native: Not detected (check if project is React Native)"
  ((WARNINGS++))
fi
...
if [ -d "node_modules" ]; then
  ...
  if [ -d "node_modules/eslint" ] || [ -f "node_modules/.bin/eslint" ]; then
    ...
    echo -e "${YELLOW}⚠️${NC}  ESLint: May not be installed (run: npm install)"
    ((WARNINGS++))
  fi
  
  if [ -d "node_modules/prettier" ] || [ -f "node_modules/.bin/prettier" ]; then
    echo -e "${GREEN}✅${NC} Prettier: Installed"
  else
    echo -e "${YELLOW}⚠️${NC}  Prettier: May not be installed (run: npm install)"
    ((WARNINGS++))
  fi
else
