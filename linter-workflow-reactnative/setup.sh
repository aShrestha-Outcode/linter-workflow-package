...
if [ -d "$PACKAGE_DIR/docs/engineering" ]; then
  ...
  # Copy all markdown files from docs/engineering
  for doc in "$PACKAGE_DIR/docs/engineering"/*.md; do
    if [ -f "$doc" ]; then
      doc_name=$(basename "$doc")
      cp "$doc" "$PROJECT_ROOT/docs/engineering/$doc_name"
      echo -e "   ${GREEN}✅${NC} Copied $doc_name"
    fi
  done
