...
This directory contains CI/CD workflows for the Outcode Flutter App using a **PR-based workflow** where quality checks run on PRs and deployment happens after merge.
...
**Note**: Protected branches (main, develop, uat, prod) don't trigger on push because:
- Direct pushes are blocked by branch protection
- Merges are already checked via PR (no need to check again)
- This prevents duplicate runs and saves GitHub Actions minutes
...
quality.yml runs automatically (on PR)
  ↓
Quality checks: format ✅, analyze ✅, metrics ✅, tests ✅
  ↓
...
See `docs/BRANCH_PROTECTION.md` for detailed setup instructions.
...
2. **Quality checks run automatically**:
   ...
   - Checks format, analyze, metrics, tests
   - Status: ✅ Pass or ❌ Fail
...
| `deploy.yml` | Generic | ⚠️ Not Active | Replaced by specific workflows |
